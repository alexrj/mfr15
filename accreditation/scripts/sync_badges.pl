...

{
    ...
    $dbix->txn(sub {
        ...
        # update all non-local badges that haven't been collected
        $dbix->dbh->do("UPDATE badges b INNER JOIN badges_import i ON (b.oid = i.oid)
            ...
            b.checkin_person_contact = i.checkin_person_contact
            WHERE b.oid IS NOT NULL AND b.checkin IS NULL");
        
        # insert new badges
        $dbix->dbh->do("INSERT IGNORE INTO badges (oid, exhibit_oid, event_oid, badge_type,
            name, lastname) SELECT oid, exhibit_oid, event_oid, badge_type,
            name, lastname FROM badges_import i
